  ...
  // Relation Information
  {
    ...
    inputs: [
      { name: "cBranchGroups", label: "Branch Groups", type: "checkbox", 
        options: 
          apiCall.branchGroupListAPIResponse.isLoading ? null : 
          apiCall.branchGroupListAPIResponse.isError ? null :
            apiCall.branchGroupListAPIResponse.isSuccess ? (
              apiCall.branchGroupListAPIResponse.data.success ? (
                apiCall.branchGroupListAPIResponse.data.list?.length > 0 ? (
                  apiCall.branchGroupListAPIResponse.data.list.map((each: any) => ({
