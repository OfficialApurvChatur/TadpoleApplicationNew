const formData = ({ apiCall }: { apiCall: any }) => ([
  // Basic Information
  {
    ...
    inputs: [
      { name: "aImage", label: "Image", type: "image", folderName: "adminprojectgroup" },
  ...
  // Relation Information
  {
    ...
    inputs: [
      { name: "cProjects", label: "Projects", type: "checkbox", 
        options: 
          apiCall.adminProjectListAPIResponse.isLoading ? null : 
          apiCall.adminProjectListAPIResponse.isError ? null :
            apiCall.adminProjectListAPIResponse.isSuccess ? (
              apiCall.adminProjectListAPIResponse.data.success ? (
                apiCall.adminProjectListAPIResponse.data.list?.length > 0 ? (
                  apiCall.adminProjectListAPIResponse.data.list.map((each: any) => ({
