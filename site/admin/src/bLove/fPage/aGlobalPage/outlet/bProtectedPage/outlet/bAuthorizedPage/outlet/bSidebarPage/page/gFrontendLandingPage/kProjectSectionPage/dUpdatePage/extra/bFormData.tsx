  ...
  retrieveAPIResponse.isSuccess ? (
    retrieveAPIResponse.data.success ? ([
      // Basic Information
      {
        ...
        inputs: [
          { name: "aImage", label: "Image", type: "image", folderName: "projectsection" },
      ...
      // Relation Information
      {
        ...
        inputs: [
          { name: "cProjectGroups", label: "Project Groups", type: "checkbox", 
            options: 
              apiCall.projectGroupListAPIResponse.isLoading ? null : 
              apiCall.projectGroupListAPIResponse.isError ? null :
                apiCall.projectGroupListAPIResponse.isSuccess ? (
                  apiCall.projectGroupListAPIResponse.data.success ? (
                    apiCall.projectGroupListAPIResponse.data.list.length > 0 ? (
                      apiCall.projectGroupListAPIResponse.data.list.map((each: any) => ({
