...
import ProjectSectionCreateComponent from "@/bLove/cComponent/aGlobalComponent/outlet/bProtectedComponent/outlet/bAuthorizedComponent/outlet/bSidebarComponent/children/gFrontendLandingComponent/kProjectSectionComponent/bCreateComponent";
...
const ProjectSectionCreatePage = () => {
  // Variable
  const navigate = useNavigate();
  const [ createAPITrigger, createAPIResponse ] = projectSectionAPIEndpoint.useProjectSectionCreateAPIMutation();
  const projectGroupListAPIResponse = projectGroupAPIEndpoint.useProjectGroupListMiniAPIQuery(null);
  ...
  const apiCall = {
    createAPITrigger,
    createAPIResponse,
    projectGroupListAPIResponse,
  }

  // JSX
  return (!isAllowedUtility(reduxCall, isAllowedConstant.projectSection, "Create") ? <UnauthorizedAccessComponent /> :
    <React.Fragment>
      {/* ProjectSectionCreatePage */}
