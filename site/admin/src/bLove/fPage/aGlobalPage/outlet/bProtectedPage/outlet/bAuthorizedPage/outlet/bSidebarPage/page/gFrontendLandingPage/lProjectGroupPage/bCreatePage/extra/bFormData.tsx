const formData = ({ apiCall }: { apiCall: any }) => ([
  // Basic Information
  {
    ...
    inputs: [
      { name: "aImage", label: "Image", type: "image", folderName: "projectgroup" },
  ...
  // Relation Information
  {
    ...
    inputs: [
      { name: "cProjects", label: "Projects", type: "checkbox", 
        options: 
          apiCall.projectListAPIResponse.isLoading ? null : 
          apiCall.projectListAPIResponse.isError ? null :
            apiCall.projectListAPIResponse.isSuccess ? (
              apiCall.projectListAPIResponse.data.success ? (
                apiCall.projectListAPIResponse.data.list?.length > 0 ? (
                  apiCall.projectListAPIResponse.data.list.map((each: any) => ({
