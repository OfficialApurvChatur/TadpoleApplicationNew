...
interface BranchSectionComponentProps {
  reduxCall: any;
  apiResponse: BranchSectionComponentDataType;
}

export const BranchSectionComponent = (props: BranchSectionComponentProps) => {
  // Destructure Props
  const { apiResponse } = props;

  // State Variable
  type TabKey = typeof apiResponse.cBranchGroups[number]["aTitle"];
  const [activeTab, setActiveTab] = useState<TabKey | undefined>(
    apiResponse?.cBranchGroups?.[0]?.aTitle
  );
          ...
          <div className="max-w-2xl mx-auto text-center py-20">
            ...
            <div className="mt-12">
              <p className="text-muted-foreground text-base">
                This section outlines the structural layers and relationships, 
                showing how different parts connect and function together.
              </p>
            </div>
          </div>

          {/* Tabs */}
          {apiResponse?.cBranchGroups && apiResponse?.cBranchGroups?.length > 0 && (
            <div className="p-4">
              ...
              {apiResponse.cBranchGroups.map((each) => (
