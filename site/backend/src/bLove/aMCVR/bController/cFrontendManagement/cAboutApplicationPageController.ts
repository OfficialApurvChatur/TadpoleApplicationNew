...
const aboutApplicationPageController = (Model= "AboutApplicationPageModel", Label= "AboutApplicationPageModel") => ({
  // Retrieve Controller
  retrieve: catchAsyncMiddleware(
    async (request: express.Request, response: express.Response, next: express.NextFunction) => {
      ...
      cacheCreateMiddleware({ 
        key: cacheVariable.aboutApplicationPageModel.retrieve({ 
          Label, 
          request 
        }), 
        data: responseData 
...
export default aboutApplicationPageController;
